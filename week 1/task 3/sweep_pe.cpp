...
int main() {
    ...
    for (double v0 = 20; v0 <= 120; v0 += 20) {
        ...
        double pe = v0 / Dr; // peclet number

        ofstream msd_file(format("msd_{:.1f}.dat", pe)); // will be plotted using python
        if (!msd_file) {
            cerr << "Error opening MSD file\n";
            return 1;
        }
        ...
        for (int i = 0; i < N; i++) {
            ...
            // initialize unwrapped & reference positions
            p[i].ux = p[i].x;
            p[i].uy = p[i].y;
            p[i].x0 = p[i].x;
            p[i].y0 = p[i].y;
